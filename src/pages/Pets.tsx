...
import { petAPI, clientAPI, rolesAPI } from '../services/api';
...
function ownerDisplayFields(owner: OwnerInfoDto | Record<string, unknown> | undefined): {
  ...
  if (!owner || typeof owner !== 'object') {
    return { ident: '', name: '', telefono: '' };
  }
  const o = owner as Record<string, unknown>;
  const ident = String(o.ident ?? o.clientIdent ?? '').trim();
  const name = String(o.name ?? o.nombre ?? '').trim();
  const telefono = String(o.telefono ?? o.phone ?? o.celular ?? '').trim();
  return { ident, name, telefono };
}

function normalizePetsPayload(raw: unknown): Pet[] {
  const arr: unknown[] = Array.isArray(raw)
    ...
    : raw && typeof raw === 'object' && 'data' in raw && Array.isArray((raw as { data: unknown }).data)
      ...
      : [];
  return arr.map((item) => {
    const p = item as Pet & { owners?: unknown[] };
    const owners = Array.isArray(p.owners)
      ? p.owners.map((ow) => {
          const x = ow as Record<string, unknown>;
          return {
            userId: (x.userId ?? x.user_id) as number | undefined,
            ident: x.ident != null ? String(x.ident) : undefined,
            name: x.name != null ? String(x.name) : x.nombre != null ? String(x.nombre) : undefined,
            telefono:
              x.telefono != null
                ? String(x.telefono)
                : x.phone != null
                  ? String(x.phone)
                  : undefined,
            correo: x.correo != null ? String(x.correo) : undefined,
          };
        })
      : p.owners;
    return { ...p, owners } as Pet;
  });
}
...
const Pets: React.FC = () => {
  ...
  const loadPets = async () => {
    try {
      ...
      const petsData = normalizePetsPayload(response.data);
      setPets(petsData);
  ...
  const filterPets = () => {
    const filtered = pets.filter(pet => {
      const ownerBlob =
        pet.owners
          ?.map((owner) => {
            const d = ownerDisplayFields(owner as OwnerInfoDto);
            return [d.ident, d.name, d.telefono].join(' ');
          })
          .join(' ') || '';
      return (
        ownerBlob.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        <div className="p-6">
          ...
          <div className="mb-4">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Buscar por identificación, nombre o celular del cliente"
                value={searchTerm}
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {paginatedData.map((pet) => {
                  const ownerIdents =
                    pet.owners
                      ?.map((owner) => ownerDisplayFields(owner as OwnerInfoDto).ident)
                      .filter(Boolean)
                      .join(', ') || '';
                  const ownerNames =
                    pet.owners
                      ?.map((owner) => ownerDisplayFields(owner as OwnerInfoDto).name)
                      .filter(Boolean)
                      .join(', ') || '';
                  const ownerPhones =
                    pet.owners
                      ?.map((owner) => ownerDisplayFields(owner as OwnerInfoDto).telefono)
                      .filter(Boolean)
                      .join(', ') || '';
